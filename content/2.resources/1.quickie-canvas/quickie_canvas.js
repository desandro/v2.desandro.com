...
$(function(){
	...
	$("#make-canvas").click(function() {
		makeCanvas();
		return false;
	});

	// jQuery UI
	$("#canvas-wrap").css( 'left', $("textarea").width() + 40 );
	
	debug('start up on init');
	
	$("textarea").resizable({
        handles: 'e, s, se',
		resize: function() {
            $("#canvas-wrap").css( 'left', $(this).width() + 40 );
            // debug('resizing');
		}
	...
	$("#bg-trans").click(function() {
		$("#canvas-wrap").css( "background-image", "none" );
	});	
	$("#bg-light").click(function() {
		$("#canvas-wrap").css( "background-image", "url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAANUlEQVRYhe3OMQ0AAAwCwfrXhqdWBEOX+4SV3GxZkmoDAAAAAAAA8A5oD9oAAAAAAAAA3gEHTl6+0+f11cwAAAAASUVORK5CYII=)" );
	});
	$("#bg-dark").click(function() {
		$("#canvas-wrap").css( "background-image", "url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAN0lEQVRYhe3PsQ0AMAzDMB/n/19qjkiBLBy0U0nyNrVdFQAAAAAAAIBzwIeDXQAAAAAAAADXgAHEipRMxjHsxgAAAABJRU5ErkJggg==)" );
	});
	
});

function makeCanvas() {
	var w = Number( $("#how-wide").val() );
	var h = Number( $("#how-tall").val() );
    ...
    $("#canvas-wrap").append( $c );

	draw( w, h);  //renders image within canvas
...
function draw( w, h){
	...
	if (canvas.getContext){
		...
		eval ( $("textarea").val() );  //grab value from the textarea and renders
